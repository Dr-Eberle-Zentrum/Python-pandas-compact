...
```{python, echo=FALSE}
last_storm_of_year = storms.sort_values(["year", "month", "day"]).drop_duplicates("status", keep="last")[["status", "year", "name"]].reset_index(drop=True)
print(last_storm_of_year)
```
...
career = pd.DataFrame({
    ...
    "degree": ["Bachelor", np.nan, np.nan, "Master", np.nan, np.nan, "Dr.", np.nan, np.nan],
    "title": ["Assistent", np.nan, np.nan, np.nan, "Gruppenleiter", np.nan, np.nan, np.nan, "Konzernchef"]
}).set_index("year")
print(career)
```
